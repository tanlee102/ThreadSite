export const nokitConvert = (data, is_article=true) => {
    ...
    data.forEach(element => {
      if(element.text){
        ...
        if(is_article)
          text = text + '<div><img class="image-nakit image-nokit" src="' + element.img + '" /></div>';
        else 
          text = text + '<a target="_blank" href="' + element.img + '"><div><img class="image-nakit image-nokit" src="' + element.img + '" /></div></a>';
