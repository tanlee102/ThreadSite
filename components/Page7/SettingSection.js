...
import ManageWordBox from '../DialogBox/ManageWordBox';

import { updateWordsByBlock } from '../../data/axios_fetch';

const SettingSection = ({listwordsbyblock}) => {

    const [displayBox, setDisplayBox] = useState(false);

    const [listWord, setListWord] = useState(listwordsbyblock);
  ...
  return (
    <div className='manage-setting-section set-border-box-1'>

    <dl class="item-select-form-account">
        <dt className='set-bold'> Từ khóa bị chặn khi đăng bài viết</dt>
        <dd>
            
            <span onClick={() => setDisplayBox(true)} className='button-icon-text'>
                <svg viewBox="0 0 194.436 194.436" >
    ...
    <ManageWordBox updateWords={updateWords} setDisplayBox={setDisplayBox} listWord={listWord} setListWord={setListWord} displayBox={displayBox}></ManageWordBox>
