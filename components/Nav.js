...
if(sorting_setting=='post' || sorting_setting==='member' || sorting_setting==='subforum' ){}else{
    ...
    <div className="nav-bar" dark-mode={darkMode ? "true" : "false"}>
        ...
        <div  className="wrap-profile" style={{display: logged ? "block" : "none"}} dark-mode={darkMode ? "true" : "false"} >
            <div className='ava-profile' onClick={() => { hideMenuProfile() }}>
                <img src=  {env_Image(userData.avatar)} alt="Logo Cuongonepiece.com"/>
            </div>
            ...
            <div ref={refOutProMenu} className='menu-profile' style={{display: menuProfile ? "block" : "none" }} onClick={() => { hideMenuProfile() }}>
                ...
                {userData.admin ?  <Link href='/manage/1'><div>Quản lý</div></Link> : ""}
                <Link href='/account/1'><div>Cài đặt</div></Link>
            </div>

        </div>

        {/*LOGIN BTN*/}
        <div style={{display: logged ? "none" : "flex"}} onClick={() => {setDisplayLoginModel(true)}} className='wrap-login-btn' dark-mode={darkMode ? "true" : "false"}>
            <div>Đăng nhập</div>
        </div>
