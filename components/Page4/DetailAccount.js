...
const DetailAccount = ({data, updateDetailUserBtn, conFirmFun}) => {
  ...
  useEffect(() => {
    ...
    if(obj['birthday'].length < 3){
      ...
      obj['year'] = Number(obj['birthday'][0]); 
      obj['month'] = Number(obj['birthday'][1]); 
      obj['day'] = Number(obj['birthday'][2]); 
    }
    setDatax(obj);
  }, []);
...
const updateAccount = () => {
  ...
  return (
    ...
    <div className='open-edit-dialog-form-account'>
      <span>Tên đăng nhập: {userData.user_name}</span>
    ...
    <div>
      ...
      <span id="date-input-form-account">
        ...
        <select value={datax.month} onChange={e => {
              obj = {...datax};
              obj['month'] = e.target.value;
              setDatax(obj);
              }} >
              {listMonth.map(item => (
                  <option key={item} selected={item === datax.month ? 'selected' : ""} value={item}> Tháng {item}</option>
              ))}
        </select>

        <input placeholder="Năm" type="number" min="1940" max="2020" value={datax['year']} 
              onChange={e => {
                obj = {...datax};
