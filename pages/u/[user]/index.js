...
const Member = ({datas, headinfo, allowed, is_block, member_id}) => {
    ...
    const addMsgBtn = () => {
      let text = String(document.getElementById('wall-comment-input').value);
      if(getLoading.current === false)
      if(text.replaceAll(/\s/g,'').length > 0){
        setLoading(true);

        $("#wall-comment-input").val("");
        document.getElementById('wall-comment-input').value = "";
            ...
            { !is_block ? 
              <div class="body-member">
                ...
                <div class="content-body-member">
                    <CommentSection deleteMsgBtn={deleteMsgBtn} deleteReMsgBtn={deleteReMsgBtn} allowed={allowed} datas={datasComment} addReMsgBtn={addReMsgBtn}  addMsgBtn={addMsgBtn} loadReMsgsBtn={loadReMsgsBtn} userData={userData} isYourProfile={member_id == userData.id}/>
                </div>
              </div>
              : 
...
export async function getServerSideProps(context) {
  ...
  if(cookies.refresh_token || context.params.user) {
      ...
      if (headinfo === undefined || headinfo === null) {headinfo = {name: 'xxx'}}
